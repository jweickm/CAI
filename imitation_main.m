...
if ~exist('./stimuli/images.mat', 'file')
    directory_1 = dir('./stimuli/1*.bmp');
    directory_2 = dir('./stimuli/2*.bmp');
    directory_startbmp = dir('./stimuli/start.bmp');
    images = cell(2,3);
...
for practiceTrial = practiceMat
    ...
    while true
        ...
        if isequal(keyCode, match_g_h)           
            ...
            if earlyRelease
                DrawFormattedText(windowPtr, uint8('Sie haben den Finger zu frueh gehoben.'), 'center', 'center', 0, 77);
                Screen('Flip', windowPtr);
...
% Display message
DrawFormattedText(windowPtr, uint8('Dies ist das Ende der Uebungstrials.'), 'center', 'center', 0, 77);
Screen('Flip', windowPtr);
disp('Press SPACEBAR or ENTER to continue.');
WaitSecs(1);
KbWait();
